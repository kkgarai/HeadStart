...
1. Resolve this engineer with `mcp__orgcs__soqlQuery`. The OrgCS Username ends with `@orgcs.com`. The user message has that username. `SELECT Id, Name, Title, Email, Username, AboutMe, Engineer_Shift__c, Manager.Name FROM User WHERE Username = '<username>' LIMIT 1`. Write `/tmp/orgcs-identity.json` as `{"name","title","manager","email","username","engineerShift","aboutMe"}`. `manager` is `Manager.Name`. `engineerShift` is `Engineer_Shift__c`. Use that Id for the case query. If `getUserInfo` errors, ignore it and keep going.
