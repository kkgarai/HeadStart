...
1. Resolve this engineer's User Id with `mcp__orgcs__soqlQuery`. The OrgCS Username ends with `@orgcs.com`. The user message has that username. `SELECT Id FROM User WHERE Username = '<username>' LIMIT 1`. Use that Id. If `getUserInfo` errors, ignore it and keep going.
